...
import { FaPaperPlane, FaPlus, FaFileUpload, FaUser, FaRobot, FaClipboardList, FaComments, FaStore, FaTrash, FaRegTrashAlt } from 'react-icons/fa';
...
const GET_SESSIONS_API_URL = `${API_BASE_URL}/api/v1/chatbot/allsessions`;
// const GET_CHATS_API_URL = `${API_BASE_URL}/api/v1/chatbot/allchats`;
...
const Message = ({ sender, text, data, intent }) => {
  ...
  const products = (intent === "search_products_result") ? data : null
  return (
    <div className={`message-wrapper ${sender}`}>
      ...
      <div className="message-content">
        ...
        {products && <ProductGrid products={products} />} </div> </div>
  );
};
...
export default function HomePage() {
  ...
  const [recievedNewSessionId, setRecievedNewSessionId] = useState(false)
...
const handleDeleteSession = async () => {
  ...
  try {
    const DELETE_SESSION = `${API_BASE_URL}/api/v1/chatbot/delete/${sessionId}`

    const res = await axios.delete(DELETE_SESSION)
  ...
  const handleSendMessage = async (userInput) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post(CHAT_API_URL, { user_id: 'c94a3f38-be17-4665-88e0-2cfe6900184c', message: userInput, credentials: credentials, context: lastBotData, session_id: sessionId });
      const { response_text, intent, data, new_session_id } = response.data;
      if (new_session_id) {
        setSessionId(new_session_id)
        setRecievedNewSessionId(true)
      }
      setLastBotData(data || null);
      console.log("79 data", data)
      setMessages([...newMessages, { sender: 'bot', text: response_text, intent: intent, data: data }]);
    }
    catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchChats = async () => {
      const GET_CHATS_API_URL = `${API_BASE_URL}/api/v1/chatbot/allchats/${sessionId}`
      const res = await axios.get(GET_CHATS_API_URL)
      ...
      const formattedMessages = res.data.map(chat => {
        ...
        if (typeof content === "string") {
          try {
            ...
            content = { text: chat.content, products: null };
          }
        }

        return {
          sender: chat.role === "ai" ? "bot" : "user",
          text: content.text ?? "",
          data: content.products ?? null,
          intent: chat.intent ?? ""
        }
      })
  ...
  const fetchSessions = useCallback(async () => {
    const res = await axios.get(GET_SESSIONS_API_URL)
    setSessions(res.data)
  }, [recievedNewSessionId])

  useEffect(() => {
    if (isConnected) {
      fetchSessions()
    }
  }, [count, isConnected, fetchSessions])
      ...
      <aside className="sidebar">
        <div className="sidebar-content">
          <div className='session-header'>
            <div className="sidebar-title"><FaComments /> Chat Sessions</div>
            <div className='new-chat' onClick={() => handleNewChat()}><FaPlus /></div>
          </div>
          <div className='' style={{
            display: "flex",
            ...
            {sessions &&
              sessions.map((d) => {
                return <button key={d.session_id} className="new-chat-button" onClick={(e) => handleChatSession(e, d.session_id)}>
                  <div className='chat-name'>
                    <div className='title-name'>
                      {d.title}
                    </div>
                   
                    <div className='trash-icon' onClick={() => handleDeleteSession()}><FaRegTrashAlt color='#d51717b1' size={14} /></div>
                  </div>
